...
describe("scene tools", () => {
  ...
  it("list_scenes fails for an unknown chapter", async () => {
    const result = await executeTool(
      "list_scenes",
      { chapterId: "00000000-0000-4000-8000-deadbeefdead" },
      ctx,
    );
    expect(result.success).toBe(false);
    expect(result.message).toMatch(/not found/i);
  ...
  it("update_scene fails for an unknown scene", async () => {
    const result = await executeTool(
      "update_scene",
      { id: "00000000-0000-4000-8000-deadbeefdead", title: "x" },
      ctx,
    );
    expect(result.success).toBe(false);
    expect(result.message).toMatch(/not found/i);
