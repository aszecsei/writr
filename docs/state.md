...
- **`projectStore`** — active project id, set once by the project layout. Every reader derives title/mode from the live `Project` row via `useActiveProject()` (`src/hooks/data/useProject.ts`).
