...
Comments are stored in Dexie with `fromOffset`/`toOffset` (1-indexed ProseMirror positions). The `Comments` extension maps those positions through doc changes via `Mapping`. A whole-document replacement (seed, reseed, staged-edit apply, version restore) must go through `replaceEditorContent` (`src/lib/editor/replace-content.ts`), which tags the transaction with `COMMENTS_RESET_META` so the plugin drops its tracked positions instead of mapping them through the replace (which would collapse every comment to the document end); `useEditorCommentSync`'s `resetReconcile` then re-anchors from `anchorText`. Point comments (`from === to`) render as markers; selection comments (`from < to`) render as highlights. Reconciliation utilities live in `src/lib/comments/`.
